const Detalle = (() => {
  async function abrir(acta) {
    ...
    try {
      ...
      try {
        ...
        if (grilla) {
          renderDetalle(body, { acta, grilla, denominacion: grilla.denominacion, titular: grilla.titulares, clase: grilla.clase, estado: grilla.estado, reivindicaciones: null, logo_url: null, expediente_url: `inpi-grilla.html?acta=${acta}`, fuente: 'INPI WS (solo grilla, detalle no disponible)', fetched_at: new Date().toISOString() }, false);
          return;
        }
      } catch {}
      body.innerHTML = `<div style="color:var(--danger); padding:12px; border:1px solid var(--danger-border); border-radius:6px; background:var(--danger-bg);">No se pudo cargar el detalle completo (INPI no respondió: ${UI.escapeHtml(e.message)}).<br>Probá <a href="https://portaltramites.inpi.gob.ar/MarcasConsultas/Grilla" target="_blank" style="text-decoration:underline;">buscar directo en el INPI</a> con acta <span class="mono">${UI.escapeHtml(acta)}</span> o reintentá en unos segundos.<br><a href="inpi-grilla.html?acta=${encodeURIComponent(acta)}" target="_blank" style="text-decoration:underline; margin-top:6px; display:inline-block;">Abrir protección en INPI ↗</a></div>`;
    }
  }
  ...
  async function fetchFresco(acta, body) {
    ...
    if (!detalle) {
      detalle = { acta, denominacion: null, titular: null, clase: null, estado: null, reivindicaciones: null, logo_url: null, expediente_url: `inpi-grilla.html?acta=${acta}`, fuente: 'INPI WS (fallback)', fetched_at: new Date().toISOString() };
    }
    if (!grilla) {
    ...
    if (!grilla) {
      ...
      warn.innerHTML = `No se pudo cargar la grilla completa desde el INPI para esta acta. Probá <a href="inpi-grilla.html?acta=${encodeURIComponent(acta)}" target="_blank" style="text-decoration:underline;">ver grilla en INPI ↗</a> con acta ${UI.escapeHtml(acta)}.`;
  ...
  function renderDetalle(el, d, cached) {
    if (!d || typeof d !== 'object' || !d.acta) {
      ...
      el.innerHTML = `<div style="color:var(--danger); padding:12px; border:1px solid var(--danger-border); border-radius:6px; background:var(--danger-bg);">No se pudo cargar el detalle para acta ${UI.escapeHtml(actaSafe)}. Probá <a href="inpi-grilla.html?acta=${encodeURIComponent(actaSafe)}" target="_blank" style="text-decoration:underline;">ver en INPI ↗</a> (Resultado completo).</div>`;
      return;
    }
