...
const CRM = (() => {
          ...
          <tbody>
            ...
            return `
                <tr>
                  ...
                  <td>${m.numero_acta ? `<button class="btn btn--ghost btn--sm" style="font-size:0.75rem; padding:2px 6px;" onclick="Detalle.abrir('${m.numero_acta}')" title="Ver grilla completa INPI">${UI.escapeHtml(m.numero_acta)} 👁️</button> <a href="inpi-grilla.html?acta=${encodeURIComponent(m.numero_acta)}" target="_blank" style="font-size:0.7rem; text-decoration:underline;">INPI ↗</a>` : '<span class="mono">—</span>'}</td>
                  <td>
    ...
    async function abrirFicha(id) {
      ...
      <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(140px,1fr)); gap: var(--space-md); margin-bottom: var(--space-lg);">
        ...
        <div><div class="form-label">N° Acta</div><div style="font-size:0.875rem;">${m.numero_acta ? `<button class="btn btn--ghost btn--sm" onclick="Detalle.abrir('${m.numero_acta}')" style="padding:2px 6px;">${UI.escapeHtml(m.numero_acta)} 👁️ grilla</button> <a href="inpi-grilla.html?acta=${encodeURIComponent(m.numero_acta)}" target="_blank" style="font-size:0.75rem; text-decoration:underline;">INPI ↗</a>` : '—'}</div></div>
